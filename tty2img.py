...
def tty2img(
        ...
        cursorColor=fgDefaultColor
        if terminalTheme:
            theme = loadTerminalTheme(terminalTheme)
            if theme:
                fgDefaultColor=theme.primary('foreground', fgDefaultColor)
                bgDefaultColor=theme.primary('background', bgDefaultColor)
...
class Theme:
    def __init__(self, yamltheme):
        self.yamltheme = yamltheme

    def primary(self, colorname, fallback):
        if 'primary' in self.yamltheme:
            if colorname in self.yamltheme['primary']:
                return _convertColor(self.yamltheme['primary'][colorname])
        return _convertColor(fallback)

    def color(self, colorname):
        if colorname.startswith('bright'):
            shortname = colorname[6,]
            if shortname in self.yamltheme['bright']:
                return _convertColor(self.yamltheme['bright'][shortname])
        else:
            if colorname in self.yamltheme['normal']:
                return _convertColor(self.yamltheme['normal'][colorname])

        return _convertColor(colorname)


def loadTerminalTheme(themeName):
    with open(themeName) as file:
